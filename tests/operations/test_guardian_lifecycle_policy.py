"""TH Guardian Lifecycle Policy mechanism (decision classification track 4, 2026-09-25).

Category B of the standing rule: the guardian lifecycle policy —
merge/activation, delegation + pre-admission proxy, consent/evidence/
expiry, records/recording rights — is owner policy, never an
engineering constant (O-D4; SEC-GUARDIAN-01's narrow containment
stands untouched). The carrier holds exactly one policy; versions carry
the effective-dated owner choices. Reads fail closed (no row / retired /
no effective version all resolve to {}), and nothing changes
identity/guardian behavior. Loads the REAL operations guardian-lifecycle
module, the REAL parent controller and the REAL academic + configuration
rules against a scripted frappe stub; hosted CI proves the journey. A
static guard also pins the containment guards to zero policy-carrier
references.
"""
import ast
...
MODULE = APP / "operations/guardian_lifecycle.py"
CONTROLLER = APP / ("operations/doctype/th_guardian_lifecycle_policy/"
                    "th_guardian_lifecycle_policy.py")
POLICY_JSON = APP / ("operations/doctype/th_guardian_lifecycle_policy/"
                     "th_guardian_lifecycle_policy.json")
VERSION_JSON = APP / ("operations/doctype/"
                      "th_guardian_lifecycle_policy_version/"
                      "th_guardian_lifecycle_policy_version.json")
FOUNDATION_SECURITY = REPO / "apps/foundation_security"
ACTOR = "course.owner@example.com"
POLICY = "TH Guardian Lifecycle Policy"
COMMANDS = (
    "create_guardian_lifecycle_policy",
    "set_guardian_lifecycle_policy_version",
    "set_guardian_lifecycle_policy_status",
    "validate_guardian_lifecycle_policy",
)

RIGHTS_TEXT = ("Guardians may view their linked students' attendance and "
               "placement records; records of other students are never "
               "visible, and recording rights stay with the office.")
...
def _install_command_harness(test):
    ...
    test.lifecycle = _load_real("toefl_house.operations.guardian_lifecycle",
                                MODULE)
    test.controller = _load_real(
        "toefl_house.operations.doctype.th_guardian_lifecycle_policy."
        "th_guardian_lifecycle_policy", CONTROLLER)
    test.foundation = sys.modules["toefl_house.configuration.rules"]


class GuardianLifecyclePolicyTests(unittest.TestCase):
    def setUp(self):
        _install_command_harness(self)

    def _existing_policy(self, status="Active", versions=()):
        self.policy_rows = [{"name": "GRD-POL"}]
        self.policy_doc = _PolicyDoc(name="GRD-POL", code="GRD-POL",
                                     title="Lifecycle shell", status=status,
                                     versions=[_Row(v) for v in versions])
        return self.policy_doc

    def _version(self, key, date, window=90, proxy="Allowed",
                 evidence="Both", expiry=365, rights=RIGHTS_TEXT):
        return self.lifecycle.set_guardian_lifecycle_policy_version(
            key, "GRD-POL", date, "Owner sets the guardian lifecycle terms",
            window, proxy, evidence, expiry, rights)

    # --- guarded commands --------------------------------------------
    def test_create_shell_carries_no_versions(self):
        result = self.lifecycle.create_guardian_lifecycle_policy(
            "test-key-guardian-create01", "GRD-POL", "Lifecycle shell")
        self.assertEqual(result["code"], "GRD-POL")
        self.assertEqual(result["status"], "Active")
        self.assertEqual(result["version_count"], 0)
        self.assertEqual(result["governing_delegation_window_days"], "")
        kind, _key, _payload = self.execute_calls[0]
        self.assertEqual(kind, "create_guardian_lifecycle_policy")

    def test_second_policy_is_refused(self):
        self._existing_policy()
        with self.assertRaises(_ValidationError) as ctx:
            self.lifecycle.create_guardian_lifecycle_policy(
                "test-key-guardian-create02", "GRD-POL-2", "Second shell")
        self.assertIn("version it instead", str(ctx.exception))

    def test_version_append_governs_from_its_date(self):
        self._existing_policy()
        result = self._version("test-key-guardian-versn01", "2026-09-28")
        self.assertEqual(result["version_count"], 1)
        # Not yet effective today: the shell still governs nothing.
        self.assertEqual(result["governing_delegation_window_days"], "")
        self.assertEqual(self.lifecycle.governing_guardian_lifecycle(), {})
        self.today = "2026-09-28"
        terms = self.lifecycle.governing_guardian_lifecycle()
        self.assertEqual(terms["effective_from"], "2026-09-28")
        self.assertEqual(terms["delegation_window_days"], 90)
        self.assertEqual(terms["pre_admission_proxy"], "Allowed")
        self.assertEqual(terms["consent_evidence"], "Both")
        self.assertEqual(terms["consent_expiry_days"], 365)
        self.assertEqual(terms["records_recording_rights"], RIGHTS_TEXT)

    def test_delegation_window_is_bounded(self):
        self._existing_policy()
        for bad in (None, 0, -1, 3651, True, "longer"):
            with self.subTest(bad=bad):
                with self.assertRaises(_ValidationError) as ctx:
                    self._version("test-key-guardian-versn02", "2026-09-28",
                                  window=bad)
                self.assertIn("Delegation window days must be a whole "
                              "number between 1 and 3650",
                              str(ctx.exception))

    def test_proxy_and_evidence_have_no_defaults(self):
        self._existing_policy()
        for bad in ("Sometimes", None, "", "allowed"):
            with self.subTest(bad=bad):
                with self.assertRaises(_ValidationError) as ctx:
                    self._version("test-key-guardian-versn03", "2026-09-28",
                                  proxy=bad)
                self.assertIn("Pre-admission proxy must be one of "
                              "Allowed / Refused; it has no default",
                              str(ctx.exception))
        for bad in ("Verbal", None, "", "both"):
            with self.subTest(bad=bad):
                with self.assertRaises(_ValidationError) as ctx:
                    self._version("test-key-guardian-versn04", "2026-09-28",
                                  evidence=bad)
                self.assertIn("Consent evidence must be one of "
                              "Link / Attachment / Both; it has no default",
                              str(ctx.exception))

    def test_consent_expiry_is_bounded(self):
        self._existing_policy()
        for bad in (None, 0, -3, 3651, True, "later"):
            with self.subTest(bad=bad):
                with self.assertRaises(_ValidationError) as ctx:
                    self._version("test-key-guardian-versn05", "2026-09-28",
                                  expiry=bad)
                self.assertIn("Consent expiry days must be a whole number "
                              "between 1 and 3650", str(ctx.exception))

    def test_records_rights_text_is_required_and_bounded(self):
        self._existing_policy()
        for bad in ("", "   ", None, "x" * 2001):
            with self.subTest(bad=str(bad)[:20]):
                with self.assertRaises(_ValidationError) as ctx:
                    self._version("test-key-guardian-versn06", "2026-09-28",
                                  rights=bad)
                self.assertIn("Records and recording rights text is "
                              "required", str(ctx.exception))

    def test_numeric_strings_are_coerced_not_assumed(self):
        self._existing_policy()
        result = self._version("test-key-guardian-versn07", "2026-09-28",
                               window="90", expiry="365")
        self.assertEqual(result["version_count"], 1)
        row = self.policy_doc.versions[0]
        self.assertEqual(row["delegation_window_days"], 90)
        self.assertEqual(row["consent_expiry_days"], 365)

    def test_backdated_and_same_day_versions_refused(self):
        self._existing_policy(versions=[{
            "effective_from": "2026-09-28", "delegation_window_days": 90,
            "pre_admission_proxy": "Allowed", "consent_evidence": "Both",
            "consent_expiry_days": 365,
            "records_recording_rights": RIGHTS_TEXT,
            "reason": "Owner sets the guardian lifecycle terms",
            "set_by": ACTOR, "set_on": "2026-09-25 12:00:00"}])
        with self.assertRaises(_ValidationError):
            self._version("test-key-guardian-versn08", "2026-09-20")
        with self.assertRaises(_ValidationError):
            self._version("test-key-guardian-versn09", "2026-09-28")
    ...
    def test_superseded_version_is_closed_not_rewritten(self):
        self._existing_policy(versions=[{
            ...
            "reason": "Owner sets the guardian lifecycle terms",
            "set_by": ACTOR, "set_on": "2026-09-25 12:00:00"}])
        self._version("test-key-guardian-versn10", "2026-10-01", window=120)
        first, second = self.policy_doc.versions
        self.assertEqual(first["delegation_window_days"], 90)
        self.assertEqual(first["superseded_on"], "2026-10-01")
        self.assertEqual(second["delegation_window_days"], 120)
        self.assertNotIn("superseded_on", second)

    def test_retire_is_the_off_switch(self):
        self._existing_policy(versions=[{
            "effective_from": "2026-09-20", "delegation_window_days": 90,
            "pre_admission_proxy": "Allowed", "consent_evidence": "Both",
            "consent_expiry_days": 365,
            "records_recording_rights": RIGHTS_TEXT,
            "reason": "Owner sets the guardian lifecycle terms",
        ...
        self.assertEqual(
            self.lifecycle.governing_guardian_lifecycle()[
                "delegation_window_days"], 90)
        result = self.lifecycle.set_guardian_lifecycle_policy_status(
            "test-key-guardian-status1", "GRD-POL", 0)
        self.assertEqual(result["status"], "Retired")
        self.assertEqual(self.lifecycle.governing_guardian_lifecycle(), {})
        self.lifecycle.set_guardian_lifecycle_policy_status(
            "test-key-guardian-status2", "GRD-POL", 1)
        self.assertEqual(
            self.lifecycle.governing_guardian_lifecycle()[
                "records_recording_rights"], RIGHTS_TEXT)
    ...
    def test_validate_requires_versions(self):
        self._existing_policy()
        with self.assertRaises(_ValidationError) as ctx:
            self.lifecycle.validate_guardian_lifecycle_policy(
                "test-key-guardian-valid01", "GRD-POL")
        self.assertIn("no versions yet", str(ctx.exception))

    def test_validate_rechecks_every_stored_row(self):
        self._existing_policy(versions=[{
            "effective_from": "2026-09-20", "delegation_window_days": 0,
            "pre_admission_proxy": "Allowed", "consent_evidence": "Both",
            "consent_expiry_days": 365,
            "records_recording_rights": RIGHTS_TEXT,
            "reason": "Owner sets the guardian lifecycle terms",
        ...
        with self.assertRaises(_ValidationError) as ctx:
            self.lifecycle.validate_guardian_lifecycle_policy(
                "test-key-guardian-valid02", "GRD-POL")
        self.assertIn("Delegation window days must be a whole number",
                      str(ctx.exception))
        self._existing_policy(versions=[{
            "effective_from": "2026-09-20", "delegation_window_days": 90,
            "pre_admission_proxy": "Allowed", "consent_evidence": "Both",
            "consent_expiry_days": None,
            "records_recording_rights": RIGHTS_TEXT,
            "reason": "Owner sets the guardian lifecycle terms",
            "set_by": ACTOR, "set_on": "2026-09-19 12:00:00"}])
        with self.assertRaises(_ValidationError) as ctx:
            self.lifecycle.validate_guardian_lifecycle_policy(
                "test-key-guardian-valid03", "GRD-POL")
        self.assertIn("Consent expiry days must be a whole number",
                      str(ctx.exception))
    ...
    def test_validate_snapshot_recovers_readiness(self):
        self._existing_policy(versions=[{
            ...
            "reason": "Owner sets the guardian lifecycle terms",
            "set_by": ACTOR, "set_on": "2026-09-25 12:00:00"}])
        result = self.lifecycle.validate_guardian_lifecycle_policy(
            "test-key-guardian-valid04", "GRD-POL")
        ...
        self.assertEqual(kind, "validate_guardian_lifecycle_policy")

    def test_all_four_commands_route_through_the_receipted_gate(self):
        self.lifecycle.create_guardian_lifecycle_policy(
            "test-key-guardian-route01", "GRD-POL", "Lifecycle shell")
        self.policy_rows = [{"name": "GRD-POL"}]
        self.policy_doc = _PolicyDoc(name="GRD-POL", code="GRD-POL",
                                     title="Lifecycle shell", status="Active",
                                     versions=[])
        self.lifecycle.set_guardian_lifecycle_policy_version(
            "test-key-guardian-route02", "GRD-POL", "2026-09-28",
            "Owner sets the guardian lifecycle terms", 90, "Allowed",
            "Both", 365, RIGHTS_TEXT)
        self.lifecycle.set_guardian_lifecycle_policy_status(
            "test-key-guardian-route03", "GRD-POL", 0)
        self.lifecycle.set_guardian_lifecycle_policy_status(
            "test-key-guardian-route04", "GRD-POL", 1)
    ...
    def _doc(self, versions=()):
        return _PolicyDoc(code="GRD-POL", title="Lifecycle shell",
    ...
    def test_two_versions_sharing_one_effective_date_refused(self):
        doc = self._doc(versions=[
            ...
            {"effective_from": "2026-09-28", "delegation_window_days": 30,
             "pre_admission_proxy": "Refused", "consent_evidence": "Link",
...
class GuardianLifecycleWiringTests(unittest.TestCase):
    ...
    def test_version_json_shape(self):
        ...
        fields = {field["fieldname"]: field for field in doc["fields"]}
        proxy = fields["pre_admission_proxy"]
        self.assertEqual(proxy.get("fieldtype"), "Select")
        self.assertEqual(proxy.get("options"), "Allowed\nRefused")
        self.assertEqual(proxy.get("reqd"), 1)
        evidence = fields["consent_evidence"]
        self.assertEqual(evidence.get("fieldtype"), "Select")
        self.assertEqual(evidence.get("options"), "Link\nAttachment\nBoth")
        self.assertEqual(evidence.get("reqd"), 1)
        for name in ("delegation_window_days", "consent_expiry_days"):
            self.assertEqual(fields[name].get("fieldtype"), "Int", name)
            self.assertEqual(fields[name].get("reqd"), 1, name)
        self.assertEqual(fields["records_recording_rights"].get("fieldtype"),
                         "Small Text")
        self.assertEqual(fields["records_recording_rights"].get("reqd"), 1)
        # No owner choice ships: no default is pinned on any value field,
        # least of all on the enumerated Select fields.
        for name in ("delegation_window_days", "pre_admission_proxy",
                     "consent_evidence", "consent_expiry_days",
                     "records_recording_rights", "reason", "effective_from"):
    ...
    def test_permission_hooks_are_wired(self):
        ...
        self.assertIn(
            "operations.doctype.th_guardian_lifecycle_policy."
            "th_guardian_lifecycle_policy.validate", validate_path)
    ...
    def test_module_and_registries_agree_on_command_kinds(self):
        ...
        whitelisted = {node.name for node in tree.body
                       ...
                       ("validate_terms", "governing_guardian_lifecycle")}
        self.assertEqual(whitelisted, set(COMMANDS))

    def test_desk_projection_is_bounded(self):
        self.assertIn('("configuration", "TH Guardian Lifecycle Policy")',
                      self.desk_init_source)
        self.assertIn(
            '("configuration", "TH Guardian Lifecycle Policy Version")',
            self.desk_init_source)
        # The enumerations and numbers project; the owner-authored
        # records/recording-rights text stays document-only.
        version_projection = self.desk_init_source.split(
            '("configuration", "TH Guardian Lifecycle Policy Version")', 1)[1]
        closing = version_projection.index("],")
        for name in ("delegation_window_days", "pre_admission_proxy",
                     "consent_evidence", "consent_expiry_days"):
            self.assertIn(name, version_projection[:closing], name)
        self.assertNotIn("records_recording_rights",
    ...
    def test_no_owner_value_is_synthesized_anywhere(self):
        ...
        self.assertIn("DELEGATION_WINDOW_MAX = 3650", self.module_source)
        self.assertIn("RECORDS_RIGHTS_MAX = 2000", self.module_source)
        # The resolver returns exactly configured terms, never substitutes.
        self.assertIn("return {}", self.module_source)
        # No policy literal is embedded as a default anywhere.
        self.assertNotIn("default_", self.module_source)

    def test_containment_guards_never_read_the_policy_carrier(self):
        # SEC-GUARDIAN-01 stands untouched: the authorization/containment
        # guards are engineering surfaces and never read owner business
        # policy. Pin every foundation_security source to zero carrier
        # references.
        for path in FOUNDATION_SECURITY.rglob("*.py"):
            source = path.read_text(encoding="utf-8")
            self.assertNotIn("TH Guardian Lifecycle Policy", source,
                             path.name)
            self.assertNotIn("guardian_lifecycle", source, path.name)
