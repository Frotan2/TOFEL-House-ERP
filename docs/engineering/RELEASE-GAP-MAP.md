...
| D10 | Desk workspaces for API-first staff roles (invigilator, placement author/publisher, admission, enrollment, teaching): the pinned frappe module-visibility gate makes workspaces reachable only for users with at least one native document read in the workspace's module. Under A13 containment these roles hold zero document permissions, so no workspace can surface for them without granting native reads (a containment change) or building a report/page-based surface per role. Owner/security decision required before any staff Desk navigation ships | Staff-facing Desk workspaces (R1 remainder) |
...
| T1 | D2 teaching compensation: `TH Instructor Contract` (+skill-term/adjustment children) + `TH Teaching Assignment` doctypes, fixtures, guarded command surface | Owner answer 2026-09-16 | next |
| T2 | D2 hosted checks: multi-instructor/skill facts, effective dating, duplicate-payable prevention, idempotent calc, fixed-salary exclusion, audit chain | T1 merged (shares suite) | — |
...
| T4 | D3 framework: guarded correction/refund command framework, approval terms configurable (owner terms pending) | — | after T1 |
...
D7 defer · D8 role-based charter (T5) · D9 CLOSED at (d) · D10 (ii)
unlocked (T3). Details: OWNER-DECISIONS.md answers table;
