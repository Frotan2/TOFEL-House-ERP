...
`tools/placement/native_checks.py` (573 checks) · Desks:
...
| Native list | branch-A class for a UP-scoped Instructor | branch-B + branchless classes hidden | role read × user permissions (`get_list`/`DatabaseQuery`) |
| Desk projection | assigned classes incl. branchless ones | off-roster learners, other teachers' classes | audience + assignment join; ignores user permissions (asserted) |
...
Two boundaries are asserted, not hidden:
...
1. The desk path (`frappe.get_all`, elevated) does not honor branch
   ...
   four groups and the desk still shows the branchless assigned class.
...
grading rule was introduced to make the journey readable.
