"""Controller for TH Alerting Policy: commands are the ONLY write path.

The guarded toefl_house.operations.alerting commands mutate this master
...
from toefl_house.operations import alerting


class THAlertingPolicy(Document):
    def validate(self):
        validate(self)
...
def validate(doc, method=None):
    ...
    try:
        foundation.assert_command_context(_(
            "Alerting policies change only through the guarded Course "
    ...
    rules.validate_title(doc.title or "", "Alerting policy title")
    ...
    for row in doc.get("versions") or []:
        ...
        foundation.validate_change_reason(row.get("reason") or "",
                                          "Version reason")
        alerting.validate_terms(row.get("channel_kind") or "",
...
def _assert_versions_sound(doc):
    ...
    if len(effective) != len(set(effective)):
        frappe.throw(_(
            "Two alerting policy versions cannot share one effective date"))
    for row in (doc.get("versions") or []):
