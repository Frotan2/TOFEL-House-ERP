"""Controller for TH Metric Stewardship Policy: commands are the ONLY write path.

The guarded toefl_house.operations.metric_stewardship commands mutate
...
from toefl_house.operations import metric_stewardship as stewardship


class THMetricStewardshipPolicy(Document):
    def validate(self):
        validate(self)
...
def validate(doc, method=None):
    ...
    try:
        foundation.assert_command_context(_(
            "Metric-stewardship policies change only through the "
    ...
    rules.validate_title(doc.title or "", "Metric-stewardship policy title")
    ...
    for row in doc.get("versions") or []:
        ...
        foundation.validate_change_reason(row.get("reason") or "",
                                          "Version reason")
        stewardship.validate_terms(row.get("steward_role") or "",
...
def _assert_versions_sound(doc):
    ...
    if len(effective) != len(set(effective)):
        frappe.throw(_("Two metric-stewardship policy versions cannot share one effective date"))
    for row in (doc.get("versions") or []):
