"""Controller for TH Roster Change Policy: commands are the ONLY write path.

The guarded toefl_house.teaching.policies commands mutate this master
...
from toefl_house.teaching import policies


class THRosterChangePolicy(Document):
    def validate(self):
        validate(self)
...
def validate(doc, method=None):
    ...
    try:
        foundation.assert_command_context(_(
            "Roster-change policies change only through the guarded "
    ...
    rules.validate_title(doc.title or "", "Roster-change policy title")
    ...
    for row in doc.get("versions") or []:
        ...
        foundation.validate_change_reason(row.get("reason") or "",
                                          "Version reason")
        policies.validate_cutoff(row.get("changes_allowed_until") or "",
