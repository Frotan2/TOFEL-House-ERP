"""Controller for TH Attendance Correction Policy: commands are the ONLY write path.

The guarded toefl_house.teaching.attendance_corrections commands mutate
this master exclusively, from inside the command context. The Course
Owner holds native write permission only so the form stays reachable;
this controller refuses any save attempted outside a configuration
command — native form, REST API, or data import — so no version can
ever bypass its mandatory change reason and hash-chained audit. Inside
a command, the rules below bind on every write.
"""
import frappe
...
from toefl_house.teaching import attendance_corrections as corrections


class THAttendanceCorrectionPolicy(Document):
    def validate(self):
        validate(self)
...
def validate(doc, method=None):
    ...
    try:
        foundation.assert_command_context(_(
            "Attendance-correction policies change only through the "
    ...
    rules.validate_title(doc.title or "", "Attendance-correction policy title")
    ...
    for row in doc.get("versions") or []:
        ...
        foundation.validate_change_reason(row.get("reason") or "",
                                          "Version reason")
        corrections.validate_terms(row.get("approver_role") or "",
...
def _assert_versions_sound(doc):
    ...
    if len(effective) != len(set(effective)):
        frappe.throw(_("Two attendance-correction policy versions cannot share one effective date"))
    for row in (doc.get("versions") or []):
