"""Controller for TH Adjustment Posting Policy: commands are the ONLY write path.

The guarded toefl_house.teaching.adjustment_posting commands mutate
...
class THAdjustmentPostingPolicy(Document):
    def validate(self):
        validate(self)
...
def validate(doc, method=None):
    ...
    try:
        foundation.assert_command_context(_(
            "Adjustment posting policies change only through the "
    ...
    rules.validate_title(doc.title or "", "Adjustment posting policy title")
    ...
    for row in doc.get("versions") or []:
        ...
        foundation.validate_change_reason(row.get("reason") or "",
                                          "Version reason")
        posting.validate_orphan_posting(row.get("orphan_posting") or "")
...
def _assert_versions_sound(doc):
    ...
    if len(effective) != len(set(effective)):
        frappe.throw(_("Two adjustment posting versions cannot share one effective date"))
    for row in (doc.get("versions") or []):
