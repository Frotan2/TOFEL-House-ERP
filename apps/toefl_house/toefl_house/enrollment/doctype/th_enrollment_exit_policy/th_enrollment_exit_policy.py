"""Controller for TH Enrollment Exit Policy: commands are the ONLY write path.

The guarded toefl_house.enrollment.exits commands mutate this master
...
from toefl_house.enrollment import exits


class THEnrollmentExitPolicy(Document):
    def validate(self):
        validate(self)
...
def validate(doc, method=None):
    ...
    try:
        foundation.assert_command_context(_(
            "Enrollment-exit policies change only through the "
    ...
    rules.validate_title(doc.title or "", "Enrollment-exit policy title")
...
def _assert_versions_sound(doc):
    ...
    if len(effective) != len(set(effective)):
        frappe.throw(_("Two enrollment-exit policy versions cannot share one effective date"))
    for row in (doc.get("versions") or []):
