"""Controller for TH Returning Student Policy: commands are the ONLY write path.

The guarded toefl_house.admission.policies commands mutate this master
...
from toefl_house.admission import policies
from toefl_house.configuration import rules as foundation


class THReturningStudentPolicy(Document):
    def validate(self):
        validate(self)
...
def validate(doc, method=None):
    ...
    try:
        foundation.assert_command_context(_(
            "Returning-student policies change only through the guarded "
            "Course Owner commands, so every change keeps its reason and "
            "audit trail"))
    except ValueError as exc:
        raise frappe.PermissionError(str(exc)) from exc
    rules.validate_code(doc.code or "")
    rules.validate_title(doc.title or "", "Returning-student policy title")
    ...
    for row in doc.get("versions") or []:
        ...
        foundation.validate_change_reason(row.get("reason") or "",
                                          "Version reason")
        policies.validate_mode(row.get("mode") or "")
...
def _assert_versions_sound(doc):
    ...
    if len(effective) != len(set(effective)):
        frappe.throw(_("Two returning-student policy versions cannot share one effective date"))
    for row in (doc.get("versions") or []):
